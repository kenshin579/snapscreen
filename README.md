...
macOS용 스크린샷 캡처 + 주석 편집 도구입니다. 오픈소스이며, 독(Dock) 아이콘 없이 메뉴바에 상주합니다.

## 기능

- **캡처**: 전역 단축키로 영역(`⌘⇧1`) · 창(`⌘⇧2`) · 전체 화면(`⌘⇧0`) 캡처
- **주석 편집기**: 화살표 · 사각형 · 원 · 텍스트 · 블러 · 모자이크 · 번호 배지, 도구별 단축키(`A`/`R`/`O`/`T`/`G`/`B`/`N`)
- **실행 취소 / 다시 실행**: `⌘Z` / `⇧⌘Z`
- **내보내기**: 클립보드로 복사(`⌘C`), 파일로 저장(`⌘S`)
